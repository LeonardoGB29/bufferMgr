...
void bufferOperation(const std::string& operation, int pageID, BufferManager& manager) {
    ...
    manager.unpinPage(pageID);  // Despinne la página independientemente de la operación para liberar el recurso
}

// Agrega la función 'loadPageToMemory' si es necesaria para cargar explícitamente páginas
